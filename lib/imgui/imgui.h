...
// - The enum only refers to fields of ImGuiStyle which makes sense to be pushed/popped inside DebugUI code.
...
struct ImGuiIO
{
    ...
    // Inputs Behaviors
    // (other variables, ones which are expected to be tweaked within DebugUI code, are exposed in ImGuiStyle)
    ...
    ImGuiContext* Ctx;                              // Parent DebugUI context (needs to be set explicitly by parent).
...
{
    ImGuiContext*       Ctx;            // Parent DebugUI context
...
// Helper: Execute a block of code at maximum once a frame. Convenient if you want to quickly create a DebugUI within deep-nested code that runs multiple times every frame.
...
// NB: You most likely do NOT need to use draw callbacks just to create your own widget or customized DebugUI rendering,
// you can poke into the draw list for that! Draw callback may be useful for example to:
//  A) Change your GPU render state,
//  B) render a complex 3D scene inside a DebugUI element without an intermediate texture/render target, etc.
...
struct ImFont
{
    ...
    ImVector<float>             IndexAdvanceX;      // 12-16 // out //            // Sparse. Glyphs->AdvanceX in a directly indexable way (cache-friendly for CalcTextSize functions which only this this info, and are often bottleneck in large DebugUI).
